...
class OrchestratorAdapter(QObject):
    ...
    def find_godot(cls, executable: str = "", log: Optional[Any] = None,
        ...
        Si `project_root` est fourni, la version déclarée par le projet est
        privilégiée : plusieurs Godot installés côte à côte est le cas normal,
        et prendre "le premier trouvé" revient à jouer à pile ou face sur la
        version qui construira la scène.
        ...
        wanted = cls.read_project_godot_version(project_root) if project_root else None
        if wanted:
            report(f"[find_godot] Version demandée par le projet cible : Godot {wanted}")
        ...
        if path_hits:
            # Un binaire du PATH dont le nom ne porte pas de version ne peut
            # pas être écarté sur ce critère : on le garde comme repli, mais
            # on privilégie une correspondance de version si elle existe.
            for hit in path_hits:
                if wanted and cls.executable_version(hit) == wanted:
                    report(f"[find_godot] Correspondance de version dans le PATH : {hit}")
                    return hit
            report(f"[find_godot] Trouvé dans le PATH : {path_hits[0]}")
            return path_hits[0]
        ...
        for directory in search_dirs:
            ...
            for match in matches:
                if match.is_file():
                    found_builds.append(str(match))

        if found_builds:
            if wanted:
                for build in found_builds:
                    if cls.executable_version(build) == wanted:
                        report(f"[find_godot] Build correspondant à Godot {wanted} : {build}")
                        return build
                versions = ", ".join(
                    f"{Path(b).name} ({cls.executable_version(b) or 'version inconnue'})"
                    for b in found_builds
                )
                report(
                    f"[find_godot] ATTENTION : aucun binaire en version {wanted} "
                    f"(celle du projet cible). Binaires trouvés : {versions}. "
                    f"Utilisation de {found_builds[0]} — la scène pourrait être "
                    "construite avec une version différente de celle qui l'ouvrira."
                )
            else:
                report(f"[find_godot] Build officiel détecté par motif : {found_builds[0]}")
...
class MainWindow(QMainWindow):
    ...
    PAGE_RESULTS = 5
    # Page « Atelier » : toutes les opérations du framework, à l'unité.
    # Ajoutée APRÈS les pages existantes pour ne décaler aucun index déjà
    # utilisé ailleurs dans le fichier (les constantes ci-dessus sont
    # référencées à de nombreux endroits).
    PAGE_WORKSHOP = 6
    ...
    def __init__(self):
        ...
        # Journal centralisé : toutes les lignes, toutes provenances.
        self._central_log_entries: list[dict[str, Any]] = []
        self._last_global_percent: int = 0
        # Le journal est affiché par défaut, masquable via l'en-tête.
        self._central_log_visible: bool = True
        ...
        # Connexion PERMANENTE de l'orchestrateur au journal centralisé.
        # Les connexions faites au moment d'un lancement (launch_*) servent à
        # piloter l'UI d'exécution ; elles ne couvrent que ces chemins-là, si
        # bien qu'une action lancée depuis l'Atelier ou une recherche de Godot
        # ne laissait aucune trace. Cette connexion-ci, elle, ne se défait
        # jamais : tout ce que l'orchestrateur émet est journalisé, d'où qu'il
        # soit appelé.
        self.orchestrator.log.connect(
            lambda msg: self.log_event(
                re.sub(r"\x1b\[[0-9;]*m", "", str(msg)).strip(),
                level=("engine" if self._is_noisy_engine_line(str(msg))
                       else ("error" if ("ERREUR" in str(msg) or "✕" in str(msg)) else "info")),
                source="orchestrateur",
            )
        )
        self.orchestrator.step_changed.connect(
            lambda step: self.log_event(f"étape : {step}", level="step", source="orchestrateur")
        )
        self.orchestrator.failed.connect(
            lambda msg: self.log_event(str(msg), level="error", source="orchestrateur")
        )
        self.orchestrator.finished.connect(
            lambda result: self.log_event(
                "opération terminée — " + ", ".join(
                    f"{k}={v}" for k, v in dict(result).items() if k != "stats"
                ),
                level="ok", source="orchestrateur",
            )
        )
    ...
    def build_ui(self) -> None:
        ...
        self.workshop_page = self.build_workshop_page()
        ...
        # Journal centralisé — capte TOUTE opération, quelle que soit la page
        # qui l'a lancée. Les journaux spécialisés (exécution, atelier,
        # autorisations) restent en place : ils donnent le détail dans leur
        # contexte, alors que celui-ci donne la chronologie complète, seul
        # endroit où l'on voit qu'une action lancée dans l'Atelier a provoqué
        # un effet visible ailleurs.
        body_layout.addWidget(self.build_central_log_panel())
    ...
    def build_header(self) -> QWidget:
        ...
        self.log_toggle_button = QPushButton("Journal ›")
        self.log_toggle_button.setObjectName("SecondaryButton")
        self.log_toggle_button.setToolTip("Afficher ou masquer le journal centralisé")
        self.log_toggle_button.clicked.connect(self.toggle_central_log)
    ...
    def build_central_log_panel(self) -> QWidget:
        panel = QFrame()
        ...
        self.central_log_filter = QComboBox()
        # "Essentiel" par défaut : les milliers de lignes de progression du
        # moteur Godot sont bien CAPTURÉES (niveau "engine") mais masquées,
        # sinon elles noient la chronologie. "Tout" les affiche — c'est ce
        # qu'il faut pour un débogage fin.
        self.central_log_filter.addItem("Essentiel", "essential")
        self.central_log_filter.addItem("Tout (verbeux)", "all")
        self.central_log_filter.addItem("Étapes", "step")
        self.central_log_filter.addItem("Erreurs", "error")
        self.central_log_filter.currentIndexChanged.connect(self.refresh_central_log)
        header.addWidget(title)
        header.addStretch()
        header.addWidget(self.central_log_filter)
        layout.addLayout(header)

        self.central_log_view = QPlainTextEdit()
        self.central_log_view.setReadOnly(True)
        self.central_log_view.setObjectName("LogView")
        self.central_log_view.setPlaceholderText(
            "Toutes les opérations lancées depuis l'application apparaissent ici, "
            "dans l'ordre, quelle que soit la page d'origine."
        )
        layout.addWidget(self.central_log_view, 1)

        actions = QHBoxLayout()
        copy_button = QPushButton("Copier")
        copy_button.setObjectName("SecondaryButton")
        copy_button.clicked.connect(self.copy_central_log)
        save_button = QPushButton("Enregistrer")
        save_button.setObjectName("SecondaryButton")
        save_button.clicked.connect(self.save_central_log)
        clear_button = QPushButton("Vider")
        clear_button.setObjectName("SecondaryButton")
        clear_button.clicked.connect(self.clear_central_log)
    ...
    def log_event(self, message: str, level: str = "info", source: str = "") -> None:
        """Point d'entrée unique du journal centralisé.

        `level` sert au filtrage ("step", "error", "info") ; `source` indique
        quelle partie de l'application a émis la ligne, pour que la
        chronologie reste lisible quand plusieurs opérations s'enchaînent.
        """
        entry = {
            "time": now_string(), "message": str(message),
            "level": level, "source": source,
        }
        self._central_log_entries.append(entry)
        # Borne haute : un import Godot produit des milliers de lignes ; sans
        # plafond, le widget finit par ralentir toute l'interface.
        if len(self._central_log_entries) > 5000:
            del self._central_log_entries[:1000]
        if self._central_log_matches_filter(entry):
            self.central_log_view.appendPlainText(self._format_log_entry(entry))

    def _format_log_entry(self, entry: dict) -> str:
        marker = {"error": "✕", "step": "▶", "warning": "⚠",
                  "ok": "✓", "engine": "·"}.get(entry["level"], " ")
        prefix = f"[{entry['time']}] {marker}"
        if entry["source"]:
            prefix += f" ({entry['source']})"
        return f"{prefix} {entry['message']}"

    def _central_log_matches_filter(self, entry: dict) -> bool:
        mode = self.central_log_filter.currentData() if hasattr(self, "central_log_filter") else "essential"
        if mode == "all":
            return True
        if mode == "error":
            return entry["level"] in ("error", "warning")
        if mode == "step":
            return entry["level"] == "step"
        return entry["level"] != "engine"

    def refresh_central_log(self) -> None:
        lines = [self._format_log_entry(e) for e in self._central_log_entries
                 if self._central_log_matches_filter(e)]
        self.central_log_view.setPlainText("\n".join(lines))

    def clear_central_log(self) -> None:
        self._central_log_entries.clear()
        self.central_log_view.clear()

    def copy_central_log(self) -> None:
        QApplication.clipboard().setText(self.central_log_view.toPlainText())
        self.statusBar().showMessage("Journal copié dans le presse-papiers", 3000)

    def save_central_log(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Enregistrer le journal",
            str(Path.home() / f"ue2godot_log_{datetime.now():%Y%m%d_%H%M%S}.txt"),
            "Fichier texte (*.txt)",
        )
        if not path:
            return
        try:
            Path(path).write_text(
                "\n".join(self._format_log_entry(e) for e in self._central_log_entries),
                encoding="utf-8",
            )
            self.statusBar().showMessage(f"Journal enregistré : {path}", 5000)
        except OSError as exc:
            QMessageBox.warning(self, "Journal", f"Enregistrement impossible.\n\n{exc}")

    def toggle_central_log(self) -> None:
        # isVisible() renvoie False tant que la fenêtre parente n'est pas
        # affichée ; on suit donc l'état voulu explicitement plutôt que de
        # l'interroger, sinon le premier clic ne fait rien de visible.
        self._central_log_visible = not getattr(self, "_central_log_visible", True)
        self.central_log_panel.setVisible(self._central_log_visible)
        self.log_toggle_button.setText(
            "Journal ›" if self._central_log_visible else "Journal ‹"
        )
    ...
    def build_sidebar(self) -> QWidget:
        ...
        for index, title in enumerate(self.page_titles):
            descriptions = [
                ...
                "Projet + données sources",
                "Objectif, modules et options",
                "État du workflow",
                "Traitement",
                "Rapport final",
                "Toutes les opérations, à l'unité",
            ]

            indicator = StepIndicator(
                index,
                title,
                # Repli plutôt qu'IndexError : ajouter une page sans ajouter
                # sa description ne doit pas empêcher l'application de
                # démarrer (cette liste parallèle aux titres est fragile par
                # construction — elle est ici tolérante).
    ...
    def build_project_page(self) -> QWidget:
        """Stage 1: choose the framework workflow, not a technical pipeline step."""
        page = PageContainer()
        ...
        page.addWidget(
            SectionHeader(
                "01 / Action",
                "Que veux-tu faire ?",
                "Le framework peut être utilisé comme une reconstruction complète ou comme un ensemble de traitements indépendants. "
                "Choisis d'abord l'objectif global ; les options techniques restent disponibles ensuite.",
            )
        )

        card = Card(
            "Workflow",
            "Aucune fonctionnalité du framework n'est supprimée par ce choix : il détermine seulement le parcours et les étapes réellement exécutées.",
        )

        self.workflow_combo = QComboBox()
        workflow_items = [
            ("Reconstruction complète UE5 → Godot", "full_reconstruction",
             "Manifest + meshes + Landscape + Decals/VFX/Audio, puis préparation/copie et construction Godot."),
            ("Assets uniquement", "assets_only",
             "Préparer le manifest et exporter les meshes sans lancer une reconstruction de scène Godot."),
            ("Landscape uniquement", "landscape_only",
             "Préparer le manifest, exporter les meshes requis par le contrat actuel, puis exécuter le traitement Landscape."),
            ("Decals / VFX / Audio uniquement", "effects_only",
             "Préparer le manifest puis produire les données Decals/VFX/Audio disponibles dans le framework."),
            ("Pipeline personnalisé", "custom",
             "Construire le parcours à partir des modules et options avancées disponibles."),
        ]
        for label, value, description in workflow_items:
            self.workflow_combo.addItem(label, value)
        initial = self.config["construction"].get("workflow", "full_reconstruction")
        idx = self.workflow_combo.findData(initial)
        if idx < 0:
            idx = 0
        self.workflow_combo.setCurrentIndex(idx)
        self.workflow_combo.currentIndexChanged.connect(self.on_workflow_changed)
        card.add(self.workflow_combo)

        self.workflow_description = QLabel()
        self.workflow_description.setObjectName("LargeSummary")
        self.workflow_description.setWordWrap(True)
        card.add(self.workflow_description)

        self.workflow_contract = QLabel()
        self.workflow_contract.setObjectName("Hint")
        self.workflow_contract.setWordWrap(True)
        card.add(self.workflow_contract)
        page.addWidget(card)

        info = Card(
            "Principe",
            "Les traitements réellement présents dans le framework restent accessibles : aucun module n'est masqué simplement parce qu'il n'est pas le workflow principal.",
        )
        info_text = QLabel(
            "<b>Important :</b> certaines capacités sont des briques du pipeline plutôt que des exports indépendants. "
            "Par exemple, Materials, Lighting et Level Instances sont actuellement représentés dans le manifest ou consommés par d'autres traitements ; l'interface ne les présentera donc pas comme des opérations autonomes si le framework ne possède pas de step dédié. "
            "Cela évite de promettre une opération qui n'existe pas réellement."
        )
        info_text.setObjectName("LargeSummary")
        info_text.setWordWrap(True)
        info.add(info_text)
        page.addWidget(info)

        actions = QHBoxLayout()
        self.action_next_button = QPushButton("Continuer vers Projet & Sources →")
        self.action_next_button.clicked.connect(lambda: self.go_to_page(self.PAGE_SOURCES))
        actions.addStretch()
        actions.addWidget(self.action_next_button)
    ...
    def build_sources_page(self) -> QWidget:
        ...
        page.addWidget(
            SectionHeader(
                "02 / Projet & Sources",
                "Projet et sources",
                "Définis la destination et la source de travail. L'application détecte ensuite automatiquement les exports et les données déjà disponibles autour de la map.",
            )
        )
        ...
        self.next_to_options_button = QPushButton("Continuer vers Configuration →")
    ...
    def build_options_page(self) -> QWidget:
        ...
        page.addWidget(
            SectionHeader(
                ...
                "Le workflow choisi à l'étape 1 détermine le parcours. Toutes les options de reconstruction, modules et réglages avancés restent disponibles ici lorsqu'ils ont un sens pour ce workflow.",
            )
        )
    ...
    def build_reconstruction_page(self) -> QWidget:
        ...
        self.execution_placeholder_button = QPushButton("Exécuter / ouvrir le workflow")
        self.execution_placeholder_button.clicked.connect(self.launch_reconstruction)

        self.headless_build_button = QPushButton("Construire la scène (headless)")
    ...
    # ------------------------------------------------------------------
    # PAGE 7 — ATELIER : toutes les opérations du framework, à l'unité
    #
    # L'interface n'exposait jusqu'ici qu'une chaîne linéaire unique. Or le
    # framework sait faire bien plus de choses indépendamment : configurer
    # seulement le matériau du terrain, relancer seulement les décals après
    # avoir corrigé un matériau, revérifier la cohérence des JSON sans rien
    # réexporter, reconstruire sans recopier… Cette page rend chacune de ces
    # capacités atteignable seule.
    #
    # Le contenu est engendré depuis ue2godot.orchestrator.operations : une
    # opération ajoutée au registre apparaît ici sans toucher à ce code, ce
    # qui rend impossible qu'une capacité existe sans être atteignable.
    # ------------------------------------------------------------------

    def build_workshop_page(self) -> QWidget:
        ...
        page.addWidget(
            SectionHeader(
                "07 / Atelier",
                "Opérations",
                "Chaque capacité du framework, lançable seule ou composée. "
                "Rien n'est imposé : l'ordre recommandé est indiqué, mais vous "
                "restez libre de lancer ce que vous voulez.",
            )
        )
        ...
        wf_card = Card(
            "Workflows",
            "Compositions suggérées. Sélectionner un workflow coche les "
            "opérations correspondantes — vous pouvez ensuite ajuster.",
        )
        wf_row = QHBoxLayout()
        self.workshop_workflow_combo = QComboBox()
        self.workshop_workflow_combo.addItem("— Sélection libre —", "")
        for key, spec in ops.WORKFLOWS.items():
            self.workshop_workflow_combo.addItem(spec["label"], key)
        self.workshop_workflow_combo.currentIndexChanged.connect(
            self.on_workshop_workflow_changed
        )
        self.workshop_workflow_hint = QLabel("")
        self.workshop_workflow_hint.setObjectName("Hint")
        self.workshop_workflow_hint.setWordWrap(True)
        wf_row.addWidget(self.workshop_workflow_combo, 1)
        wf_card.add_layout(wf_row)
        wf_card.add(self.workshop_workflow_hint)
        page.addWidget(wf_card)

        # --- Une carte par catégorie -------------------------------------
        self.workshop_checks: dict[str, QCheckBox] = {}
        self.workshop_status: dict[str, StatusPill] = {}

        for category in ops.categories():
            card = Card(category, "")
            for op in ops.operations_in(category):
                row = QHBoxLayout()

                check = QCheckBox()
                check.setToolTip("Inclure dans l'exécution groupée")
                check.stateChanged.connect(self.refresh_workshop_warnings)
                self.workshop_checks[op.key] = check
                ...
                if op.side == ops.SIDE_UNREAL:
                    tags.append("éditeur Unreal requis")
                elif op.side == ops.SIDE_GODOT:
                    tags.append("Godot headless")
                if op.mutates_editor:
                    tags.append("modifie la map")
                if op.rewrites_output:
                    tags.append("réécrit ses fichiers")
                if op.long_running:
                    tags.append("long")
                if tags:
                    tag_label = QLabel(" · ".join(tags))
                    tag_label.setObjectName("Hint")
                    label_box.addWidget(tag_label)

                pill = StatusPill("—", "neutral")
                self.workshop_status[op.key] = pill

                run_button = QPushButton("Exécuter")
                run_button.setObjectName("SecondaryButton")
                run_button.clicked.connect(
                    lambda _checked=False, k=op.key: self.run_single_operation(k)
                )
        ...
        batch_card = Card(
            "Exécution groupée",
            "Lance les opérations cochées, dans l'ordre affiché, en "
            "s'arrêtant à la première qui échoue.",
        )
        self.workshop_warnings = QPlainTextEdit()
        self.workshop_warnings.setReadOnly(True)
        self.workshop_warnings.setObjectName("LogView")
        self.workshop_warnings.setMinimumHeight(90)
        self.workshop_warnings.setPlaceholderText(
            "Les avertissements de dépendance et d'ordre apparaîtront ici."
        )
        batch_card.add(self.workshop_warnings)

        batch_actions = QHBoxLayout()
        clear_button = QPushButton("Tout décocher")
        clear_button.setObjectName("SecondaryButton")
        clear_button.clicked.connect(self.clear_workshop_selection)
        self.workshop_run_button = QPushButton("Exécuter la sélection")
        self.workshop_run_button.clicked.connect(self.run_workshop_selection)
        batch_actions.addWidget(clear_button)
        batch_actions.addStretch()
        batch_actions.addWidget(self.workshop_run_button)
        batch_card.add_layout(batch_actions)
        page.addWidget(batch_card)

        # --- Journal ------------------------------------------------------
        log_card = Card("Journal de l'atelier", "Sortie de chaque opération lancée ici.")
        self.workshop_log = QPlainTextEdit()
        self.workshop_log.setReadOnly(True)
        self.workshop_log.setObjectName("LogView")
        self.workshop_log.setMinimumHeight(200)
        log_card.add(self.workshop_log)
    ...
    def _workshop_log(self, message: str) -> None:
        self.workshop_log.appendPlainText(f"[{now_string()}] {message}")
        text = str(message)
        level = "error" if text.lstrip().startswith("✕") else (
            "warning" if text.lstrip().startswith("⚠") else "info")
        self.log_event(text, level=level, source="atelier")

    def on_workshop_workflow_changed(self) -> None:
        from ue2godot.orchestrator import operations as ops

        key = self.workshop_workflow_combo.currentData()
        if not key:
            self.workshop_workflow_hint.setText("")
            return
        spec = ops.WORKFLOWS.get(key, {})
        self.workshop_workflow_hint.setText(spec.get("description", ""))
        wanted = set(spec.get("operations", []))
        for op_key, check in self.workshop_checks.items():
            check.blockSignals(True)
            check.setChecked(op_key in wanted)
            check.blockSignals(False)
        self.refresh_workshop_warnings()

    def clear_workshop_selection(self) -> None:
        for check in self.workshop_checks.values():
            check.blockSignals(True)
            check.setChecked(False)
            check.blockSignals(False)
        self.workshop_workflow_combo.setCurrentIndex(0)
        self.refresh_workshop_warnings()

    def _selected_workshop_keys(self) -> list[str]:
        from ue2godot.orchestrator import operations as ops
        return [op.key for op in ops.OPERATIONS
                if self.workshop_checks.get(op.key) is not None
                and self.workshop_checks[op.key].isChecked()]

    def _available_artifacts(self) -> list[str]:
        """Ce qui existe RÉELLEMENT sur disque, pas ce que nous croyons avoir
        lancé : un export fait à la main dans la console Python d'Unreal doit
        compter autant qu'un export lancé depuis ici."""
        from ue2godot.orchestrator import operations as ops

        available: list[str] = []
        inv = self.inventory
        if inv.manifest:
            available.append(ops.ART_MANIFEST)
        if inv.asset_map:
            available.append(ops.ART_ASSET_MAP)
        if inv.decal_map:
            available.append(ops.ART_DECAL_MAP)
        if inv.mesh_directory:
            available.append(ops.ART_MESHES)

        project = Path(self.config["project"].get("godot_project", "")).expanduser()
        if project.is_dir():
            asset_sub = self.config["advanced"].get("godot_asset_root", "UEAssets")
            if (project / asset_sub).is_dir():
                available.append(ops.ART_COPIED)
            if (project / "addons" / "ue2godot").is_dir():
                available.append(ops.ART_RUNTIME)
            if (project / ".godot").is_dir():
                available.append(ops.ART_IMPORTED)
            for candidate in ("Map--_REBUILT.tscn", "Maps/Map_REBUILT.tscn"):
                if (project / candidate).is_file():
                    available.append(ops.ART_SCENE)
                    break
        return available

    def refresh_workshop_warnings(self) -> None:
        from ue2godot.orchestrator import operations as ops

        keys = self._selected_workshop_keys()
        if not keys:
            self.workshop_warnings.setPlainText("")
            return
        warnings = ops.validate_selection(keys, self._available_artifacts())
        if warnings:
            self.workshop_warnings.setPlainText("\n".join("⚠ " + w for w in warnings))
        else:
            self.workshop_warnings.setPlainText(
                "✓ Sélection cohérente : les entrées de chaque opération sont "
                "disponibles ou produites avant elle."
            )

    def run_single_operation(self, key: str) -> None:
        self._execute_operations([key])

    def run_workshop_selection(self) -> None:
        keys = self._selected_workshop_keys()
        if not keys:
            QMessageBox.information(
                self, "Atelier",
                "Cochez au moins une opération, ou choisissez un workflow.",
            )
            return
        ...
        if warnings:
            answer = QMessageBox.question(
                self, "Avertissements",
                "La sélection présente des avertissements :\n\n"
                + "\n".join("• " + w for w in warnings)
                + "\n\nLancer quand même ?",
            ...
            if answer != QMessageBox.Yes:
                return
        self._execute_operations(keys)

    def _execute_operations(self, keys: list[str]) -> None:
        """Dispatche chaque opération vers son transport selon son `side`.

        Les opérations côté Unreal sont groupées en UN SEUL aller-retour vers
        l'éditeur quand elles se suivent : ouvrir un canal d'exécution
        distante par étape serait à la fois plus lent et plus fragile.
        """
        from ue2godot.orchestrator import operations as ops

        selected = [ops.BY_KEY[k] for k in keys if k in ops.BY_KEY]
        mutating = [o for o in selected if o.mutates_editor]
        if mutating:
            answer = QMessageBox.question(
                self, "Modification de la map Unreal",
                "Ces opérations modifient l'état de la map ouverte dans "
                "Unreal :\n\n"
                + "\n".join("• " + o.label for o in mutating)
                + "\n\nContinuer ?",
                QMessageBox.Yes | QMessageBox.No, QMessageBox.No,
            )
            if answer != QMessageBox.Yes:
        ...
        for op in selected:
            self.workshop_status[op.key].setText("…")
            self.workshop_status[op.key].set_state("warning")
        ...
        while index < len(selected):
            ...
            # Regroupement des étapes Unreal consécutives.
            if op.side == ops.SIDE_UNREAL:
                group = []
                while index < len(selected) and selected[index].side == ops.SIDE_UNREAL:
                    group.append(selected[index])
                    index += 1
                if not self._run_unreal_group(group):
                    self._mark_remaining_skipped(selected[index:])
                    return
                continue

            ok = self._run_local_operation(op)
            index += 1
            if not ok:
                self._mark_remaining_skipped(selected[index:])
                return

        self._workshop_log("Sélection terminée.")
        self.on_source_changed()

    def _mark_remaining_skipped(self, remaining: list) -> None:
        for op in remaining:
            self.workshop_status[op.key].setText("non lancée")
            self.workshop_status[op.key].set_state("neutral")
        self._workshop_log(
            "Arrêt après échec — les opérations suivantes n'ont pas été lancées "
            "(elles dépendent du résultat de celle qui a échoué)."
        )

    def _run_unreal_group(self, group: list) -> bool:
        """Exécute un groupe d'étapes Unreal via l'exécution distante, avec
        repli copier-coller si elle est indisponible."""
        ...
        if not source_path:
            self._workshop_log("✕ Aucune map Unreal sélectionnée (onglet Sources).")
            for op in group:
                self.workshop_status[op.key].setText("bloquée")
                self.workshop_status[op.key].set_state("danger")
            return False

        res_cfg = self._build_resolved_config()
        export_dir = res_cfg.get("paths.ue_export_root", "")
        ...
        uproj = find_uproject(source_path) or ""
        ini = os.path.join(os.path.dirname(uproj), "Config", "DefaultEngine.ini") if uproj else ""
        adapter = UERemoteAdapter(res_cfg, default_engine_ini=ini)
        module_root = os.path.dirname(os.path.abspath(__file__))
        step_names = [op.step_name for op in group if op.step_name]
        ...
        if not remote:
            instructions = adapter.build_fallback_instructions(
                step_names, run_config_path, module_root=module_root,
                report_dir=report_dir,
            )
            self._workshop_log("Exécution distante indisponible — repli manuel :")
            for line in instructions:
                self._workshop_log("  " + line)
            for op in group:
                self.workshop_status[op.key].setText("à lancer dans Unreal")
                self.workshop_status[op.key].set_state("warning")
        ...
        try:
            ...
            self._workshop_log(f"✕ Exécution distante interrompue : {exc}")
            for op in group:
                self.workshop_status[op.key].setText("échec")
                self.workshop_status[op.key].set_state("danger")
            return False
        except Exception as exc:
            QApplication.restoreOverrideCursor()
            self._workshop_log(f"✕ Erreur inattendue : {exc}")
            return False
        finally:
            QApplication.restoreOverrideCursor()

        by_step = {name: (ok, msg) for name, ok, msg in results}
        all_ok = True
        for op in group:
            ok, msg = by_step.get(op.step_name, (False, "non exécutée"))
            self.workshop_status[op.key].setText("OK" if ok else "échec")
            self.workshop_status[op.key].set_state("success" if ok else "danger")
            self._workshop_log(f"{'✓' if ok else '✕'} {op.label}")
            for line in str(msg).splitlines():
                if line.strip():
                    self._workshop_log("    " + line.strip())
            if not ok:
                all_ok = False
        return all_ok

    def _build_resolved_config(self):
        """Config résolue à partir de l'état courant de l'UI."""
        ...
        return ResolvedConfig.resolve({
            "paths": {
                ...
                "godot_project_root": self.config["project"].get("godot_project", ""),
    ...
    def _run_local_operation(self, op) -> bool:
        """Opérations locales et Godot. Chacune met à jour son propre statut."""
        from ue2godot.orchestrator import operations as ops
        ...
        try:
            if op.key == "copy_assets":
                if not self.plan:
                    self.prepare_reconstruction()
                if not self.plan:
                    return done(False, "plan indisponible")
                return done(self.orchestrator.copy_assets(self.plan))

            if op.key == "deploy_runtime":
                # Le déploiement du runtime n'a pas de méthode dédiée : il est
                # effectué par copy_assets(), qui copie les données Unreal ET
                # l'addon ue2godot dans le projet cible. On appelle donc
                # copy_assets, et on vérifie ensuite la présence réelle du
                # script d'entrée plutôt que de se fier au retour seul.
                if not self.plan:
                    self.prepare_reconstruction()
                if not self.plan:
                    return done(False, "plan indisponible")
                self.orchestrator.copy_assets(self.plan)
                project = Path(self.config["project"].get("godot_project", ""))
                entry = project / "addons" / "ue2godot" / "entry_headless.gd"
                if entry.is_file():
                    return done(True, str(entry))
                return done(False, "entry_headless.gd absent du projet cible")

            if op.key == "godot_import":
                # build_scene_headless() enchaîne import PUIS construction en
                # une seule séquence asynchrone : il n'existe pas d'import
                # isolé aujourd'hui. On le dit plutôt que de faire croire que
                # seul l'import a tourné.
                if not self.plan:
                    self.prepare_reconstruction()
                if "godot_build" in [o.key for o in
                                     [ops.BY_KEY[k] for k in self._selected_workshop_keys()
                                      if k in ops.BY_KEY]]:
                    return done(True, "sera fait par « Construire la scène »")
                return done(self.orchestrator.build_scene_headless(self.plan),
                            "import puis construction lancés en arrière-plan")

            if op.key == "godot_build":
                if not self.plan:
                    self.prepare_reconstruction()
                return done(self.orchestrator.build_scene_headless(self.plan),
                            "lancé en arrière-plan ; voir le journal d'exécution")

            if op.key == "open_godot":
                if not self.plan:
                    self.prepare_reconstruction()
                return done(self.orchestrator.open_godot(self.plan))

            if op.key == "open_scene":
                self.open_result_scene()
                return done(True)

            if op.key == "crosscheck":
                return done(*self._run_crosscheck_operation())

            if op.key == "inspect_source":
                self.on_source_changed()
                lines = self.source_analyzer.describe(self.inventory) \
                    if hasattr(self.source_analyzer, "describe") else []
                self._workshop_log(f"Manifest : {self.inventory.manifest or '—'}")
                self._workshop_log(f"Asset map : {self.inventory.asset_map or '—'}")
                self._workshop_log(f"Decal map : {self.inventory.decal_map or '—'}")
                self._workshop_log(f"Dossier meshes : {self.inventory.mesh_directory or '—'}")
                for line in lines:
                    self._workshop_log("  " + str(line))
                return done(True)

            if op.key == "read_reports":
                return done(*self._read_step_reports())

            if op.key == "check_auth":
                self.check_unreal_authorizations()
                self.go_to_page(self.PAGE_PROJECT)
                return done(True, "voir l'onglet Projet")

            if op.key == "grant_auth":
                self.grant_unreal_authorizations()
                return done(True)

            if op.key == "test_connection":
                self.test_unreal_connection()
                self.go_to_page(self.PAGE_PROJECT)
                return done(True, "voir l'onglet Projet")

            if op.key == "diagnose_network":
                self.diagnose_unreal_network()
                self.go_to_page(self.PAGE_PROJECT)
                return done(True, "voir l'onglet Projet")
        ...
        return done(False, "opération non implémentée")

    def _run_crosscheck_operation(self) -> tuple:
        from ue2godot.orchestrator.pipeline import PipelineOrchestrator

        res_cfg = self._build_resolved_config()
        export_dir = res_cfg.get("paths.ue_export_root", "")
        manifest = os.path.join(export_dir, "level_manifest_v10.json")
        asset_map = os.path.join(export_dir, "GodotAssets", "ue5_godot_asset_map.json")
        decal_map = os.path.join(export_dir, "GodotAssets", "ue5_godot_decal_map.json")

        report = PipelineOrchestrator(res_cfg).crosscheck(
            manifest, asset_map, decal_map if os.path.isfile(decal_map) else None,
        )
        for warning in report.warnings:
            self._workshop_log("  ⚠ " + warning)
        for error in report.errors:
            self._workshop_log("  ✕ " + error)
        return report.status == "OK", report.status

    def _read_step_reports(self) -> tuple:
        res_cfg = self._build_resolved_config()
        report_dir = os.path.join(res_cfg.get("paths.ue_export_root", ""), "reports")
        if not os.path.isdir(report_dir):
            self._workshop_log(f"  Aucun dossier de rapports : {report_dir}")
            return False, "aucun rapport"

        found = sorted(Path(report_dir).glob("step_*.json"))
        if not found:
            self._workshop_log("  Aucun rapport d'étape trouvé.")
            return False, "aucun rapport"

        for path in found:
            data = read_json(path) or {}
            self._workshop_log(
                f"  {path.name} : {data.get('status', '?')} "
                f"— {data.get('counters', {})}"
            )
            for warning in data.get("warnings", [])[:5]:
                self._workshop_log("      ⚠ " + str(warning))
            for error in data.get("errors", [])[:5]:
                self._workshop_log("      ✕ " + str(error))
        return True, f"{len(found)} rapport(s)"
    ...
    WORKFLOW_DESCRIPTIONS = {
        "full_reconstruction": (
            "Reconstruction complète : toutes les étapes compatibles du framework sont préparées, puis les assets peuvent être copiés et la scène Godot construite.",
            "Nécessite une destination Godot valide pour la partie Godot du workflow.",
        ),
        "assets_only": (
            "Assets uniquement : le manifest et l'export des meshes sont préparés. Aucun faux résultat de scène Godot n'est produit.",
            "Ce workflow s'arrête au paquet d'assets ; la destination Godot n'est pas nécessaire pour l'export Unreal.",
        ),
        "landscape_only": (
            "Landscape uniquement : le framework prépare le manifest, les meshes requis par le step Landscape, puis exécute le traitement Landscape réel (matériau, géométrie et bake selon ses capacités).",
            "Le step Landscape actuel dépend explicitement du manifest et de l'asset map produite par le step Meshes.",
        ),
        "effects_only": (
            "Decals / VFX / Audio : le manifest est préparé puis le step Decals/VFX produit les sorties réellement supportées, sans prétendre convertir Niagara ou l'audio en fonctionnalités inexistantes.",
            "Les VFX peuvent rester en marqueurs ou utiliser le mode de substitution partiel ; l'audio reste des marqueurs uniquement.",
        ),
        "custom": (
            "Pipeline personnalisé : les modules, la fidélité et les options avancées déterminent le traitement. Les dépendances réelles entre steps sont conservées.",
            "Les cases ne créent jamais artificiellement un step qui n'existe pas dans le framework.",
        ),
    }
    ...
    def on_workflow_changed(self) -> None:
        ...
        if not workflow:
            return
        self.config["construction"]["workflow"] = workflow
        description, contract = self.WORKFLOW_DESCRIPTIONS.get(workflow, ("", ""))
        self.workflow_description.setText(description)
        self.workflow_contract.setText(contract)

        # Workflow presets change only the broad objective. The module matrix
        # remains editable and all advanced settings remain available.
        presets = {
            "full_reconstruction": "maximum",
            "assets_only": "geometry",
            "landscape_only": "environment",
            "effects_only": "custom",
            "custom": "custom",
        }
        objective = presets.get(workflow, "custom")
        idx = self.objective_combo.findData(objective)
        if idx >= 0 and workflow != "custom":
    ...
    def run_unreal_export_steps(self) -> None:
        ...
        workflow = self.config["construction"].get("workflow", "full_reconstruction")
        if workflow == "assets_only":
            selected_steps = ["manifest", "meshes"]
        elif workflow == "landscape_only":
            # step3_landscape explicitly requires step1 + step2 outputs.
            selected_steps = ["manifest", "meshes", "landscape"]
        elif workflow == "effects_only":
            selected_steps = ["manifest", "decals_vfx"]
        elif workflow == "custom":
            selected_steps = ["manifest"]
            if modules.get("landscape", False):
                selected_steps.append("meshes")
                selected_steps.append("landscape")
            # The current framework has no standalone module switch named
            # "meshes". Mesh export is exposed explicitly by the
            # Assets-only workflow; in custom mode it becomes a dependency
            # only when Landscape is selected. Do not invent a mesh step
            # from unrelated manifest-only modules.
            if modules.get("decals", False) or modules.get("vfx", False) or modules.get("audio", False):
                selected_steps.append("decals_vfx")
        else:
            selected_steps = ["manifest", "meshes", "landscape", "decals_vfx"]

        step_keys = (["preprocess"] if preprocessing_confirmed else []) + selected_steps
    ...
    def run_validation(self) -> bool:
        ...
        workflow = self.config["construction"].get("workflow", "full_reconstruction")
        if workflow in ("full_reconstruction", "landscape_only", "custom") and not self.config["source"].get("preprocessing_confirmed", False):
            errors.append(
                "Le prétraitement UE5 (détachement + LevelInstances cassées) n'a pas "
                "été confirmé sur la page Projet & Sources. Rien dans le manifest ne peut le "
        ...
        godot_required = workflow == "full_reconstruction"
        if godot_path and (Path(godot_path) / "project.godot").exists():
            ...
            self.add_validation_row(
                "Projet Godot",
                "Le projet Godot cible est requis pour la reconstruction complète mais n'est pas encore validé.",
                "warning",
            )
            warnings.append("Projet Godot non validé pour la reconstruction complète.")
        else:
            self.add_validation_row(
                "Projet Godot",
                "Non requis pour ce workflow d'export Unreal.",
                "success",
            )
        ...
        if godot_required and not (godot_path and (Path(godot_path) / "project.godot").exists()):
            errors.append("Projet Godot cible requis mais project.godot n'est pas validé.")
    ...
    def prepare_reconstruction(self) -> None:
        ...
        if self._last_validation_warnings and self._last_validation_mode != "diagnostic":
            answer = QMessageBox.question(
                ...
                "Certaines étapes d'export seront générées lors de l'exécution du workflow sélectionné.\n\n"
                "Voulez-vous préparer le plan et continuer ?",
        ...
        self.execution_logs.appendPlainText(
            f"[{now_string()}] Workflow : {self.config['construction'].get('workflow', 'full_reconstruction')}"
        )
        ...
        workflow_labels = {
            "full_reconstruction": "reconstruction complète UE5 → Godot",
            "assets_only": "export des assets uniquement",
            "landscape_only": "export/traitement Landscape uniquement",
            "effects_only": "export Decals/VFX/Audio uniquement",
            "custom": "pipeline personnalisé",
        }
        workflow_label = workflow_labels.get(self.config["construction"].get("workflow", "full_reconstruction"), "workflow")
        self.execution_summary.setText(
            f"Plan préparé pour le {workflow_label}. Les étapes réelles du framework seront exécutées selon leurs dépendances ; aucun résultat non produit ne sera déclaré comme terminé."
        )
    ...
    def launch_reconstruction(self) -> None:
        workflow = self.config["construction"].get("workflow", "full_reconstruction")
        if workflow != "full_reconstruction":
            # Standalone workflows are Unreal/export workflows. They must not
            # silently require a Godot project or pretend to have built a scene.
            self.run_unreal_export_steps()
            return
    ...
    def _on_orchestrator_log(self, message: str) -> None:
        # Journal spécialisé (détail technique complet, reste en place)…
        self.execution_logs.appendPlainText(f"[{now_string()}] {message}")

        # Le journal centralisé est alimenté par la connexion permanente
        # établie dans __init__ — ne pas réémettre ici, sinon doublon.
        self._update_progress_from_log(str(message))

    @staticmethod
    def _is_noisy_engine_line(text: str) -> bool:
        stripped = text.strip()
        return (
            stripped.startswith("[")
            and ("%" in stripped[:12] or "DONE" in stripped[:12])
        ) or stripped in ("", "\x1b[0m")

    def _update_progress_from_log(self, text: str) -> None:
        """Traduit la sortie brute de Godot en progression lisible.

        Godot émet sa progression sous la forme `[  42% ] reimport | fichier`.
        Sans cette lecture, la barre restait figée pendant les vingt minutes
        d'import et rien n'indiquait que le traitement avançait — alors que
        l'information existait, noyée dans le flot technique.
        """
        # Les séquences ANSI de couleur s'intercalent ENTRE le pourcentage et
        # le nom de phase ("[ 42% ] \x1b[90m\x1b[1mreimport\x1b[22m | ...").
        # Les retirer avant l'analyse, sinon le nom de phase n'est jamais
        # reconnu et la barre reste figée — erreur trouvée en testant sur une
        # vraie ligne de sortie Godot plutôt qu'une ligne idéalisée.
        clean = re.sub(r"\x1b\[[0-9;]*m", "", text)
        match = re.search(r"\[\s*(\d{1,3})%\s*\]\s*([A-Za-z_]+)?\s*\|?\s*(.*)", clean)
        if not match:
            return
        percent = int(match.group(1))
        phase_key = (match.group(2) or "").strip()
        detail = (match.group(3) or "").strip()

        phase_labels = {
            "reimport": "Import des assets",
            "import": "Import d'une scène",
            "first_scan_filesystem": "Analyse du projet",
            "_update_scan_actions": "Analyse des fichiers",
            "update_scripts_classes": "Enregistrement des scripts",
            "loading_editor_layout": "Chargement de l'éditeur",
        }
        phase = phase_labels.get(phase_key, phase_key or "Traitement")

        if phase_key == "reimport":
            # Seule la progression globale de la réimportation reflète
            # l'avancement réel ; celle d'une scène individuelle repart à 0
            # à chaque fichier et ferait osciller la barre sans arrêt.
            self.execution_progress.setValue(max(1, percent))
            self._last_global_percent = percent

        self.execution_phase.setText(phase)
        if detail:
            self.execution_stage.setText(detail[:90])
        if phase_key == "reimport" and detail and not detail.startswith("Prépar"):
            self.execution_placements.setText(f"{percent} % — {detail[:40]}")
    ...
    def collect_config_from_ui(self) -> None:
        ...
        self.config["construction"]["workflow"] = self.workflow_combo.currentData()
    ...
    def apply_config_to_ui(self) -> None:
        ...
        workflow_index = self.workflow_combo.findData(
            self.config["construction"].get("workflow", "full_reconstruction")
        )
        if workflow_index >= 0:
